...
namespace Library.Services.Books
{
    ...
    public class BookService : IBookService
    {
        ...
        public BookService(IBookRepository repository, ICategoryRepository categoryRepository,
            IAuthorRepository authorRepository,IUserRepository userRepository, UnitOfWork unit)
        {
        ...
        {
            var book = await _bookRepository.GetByIdAsync(bookId);
            var rentedNumber = _userRepository.CalBookRentedNumbers(bookId);
            if (book == null)
            {
                throw new NotFoundExeption("Data not found");
            }
            var newcount = book.BookCount + newBookcount;
            if (newcount < rentedNumber)
            {
